...
import { EstateFormData, initialEstateFormData } from '@/types/estate-form';
...
export function EstateForm() {
  ...
  const [formData, setFormData] = useState<EstateFormData>(initialEstateFormData);
  const [isSuccess, setIsSuccess] = useState(false);
  const { toast } = useToast();
  const { submit, isSubmitting } = useZapier({ webhookUrl: ZAPIER_WEBHOOKS.ESTATE });
    ...
    try {
      ...
      if (success) {
        toast({
          ...
          description: "Vi kommer att kontakta dig inom kort med offerter från våra samarbetspartners.",
        });
        setIsSuccess(true);
        setFormData(initialEstateFormData);
        setCurrentStep(1);
      } else {
  ...
  const updateStepData = (step: keyof EstateFormData, data: Partial<EstateFormData[keyof EstateFormData]>) => {
    setFormData((prev) => ({
      ...prev,
      [step]: {
        ...prev[step],
  ...
  return (
    <FormLayout
      title="Få hjälp med dödsbohantering"
      ...
      successDescription="Vi har mottagit din förfrågan och kommer inom kort att kontakta dig med offerter från våra kvalitetssäkrade samarbetspartners."
    >
      {renderStep()}
    </FormLayout>
  );
